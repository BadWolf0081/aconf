...
install_aegis(){
  mount_system_rw
  setup_initd_dir
if [ ! -f /data/bin/42aegis ] ;then
  until $download /data/bin/55aegis $url/scripts/55aegis || { logger "download 55aegis failed, exit script" ; exit 1; } ;do
    sleep 2
  done
  chmod +x /data/bin/55aegis
...
on property:sys.boot_completed=1
    exec_background u:r:init:s0 root root -- /data/bin/55aegis
EOF
...
if [[ $(basename $0) = "aegis_new.sh" ]] ;then
  if [[ -f /data/bin/42aegis ]] ;then
    old42=$(head -2 /data/bin/42aegis | grep '# version' | awk '{ print $NF }')
    if [ $Ver42aegis != $old42 ] ;then
      mount_system_rw
      setup_initd_dir
      until $download /data/bin/42aegis $url/scripts/42aegis || { logger "download 42aegis failed, exit script" ; exit 1; } ;do
        sleep 2
      done
      chmod +x /data/bin/42aegis
      mount_system_ro
      new42=$(head -2 /data/bin/42aegis | grep '# version' | awk '{ print $NF }')
...
if [[ $(basename $0) = "aegis_new.sh" ]] ;then
  if [[ -f /data/bin/55aegis ]] ;then
    old55=$(head -2 /data/bin/55aegis | grep '# version' | awk '{ print $NF }')
    if [ $Ver55aegis != $old55 ] ;then
      ...
      chmod +x /data/bin/55aegis
      mount_system_ro
      new55=$(head -2 /data/bin/55aegis | grep '# version' | awk '{ print $NF }')
...
# check aegis running
#aegis_check=$(ps -e | grep com.pokemod.aegis:mapping | awk '{print $9}')
