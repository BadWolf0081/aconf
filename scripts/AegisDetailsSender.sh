...
# generic
    ...
    MITMSh=$(head -2 /data/adb/service.d/aegis.sh | grep '# version' | awk '{ print $NF }')
    ...
    monitor=$([ -f /data/adb/service.d/aegis_monitor.sh ] && head -2 /data/adb/service.d/aegis_monitor.sh | grep '# version' | awk '{ print $NF }' || echo 'na')
    whversion=$([ -f /data/adb/service.d/AegisDetailsSender.sh ] && head -2 /data/adb/service.d/AegisDetailsSender.sh | grep '# version' | awk '{ print $NF }' || echo 'na')
